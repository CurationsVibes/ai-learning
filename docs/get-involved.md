# Get Involved

**Join the CURATIONS community - learn, contribute, collaborate**

---

## How To Participate

CURATIONS is a living collaboration. Here's how you can be part of it:

---

## 🌱 For Youth & Students

### Learn & Create

**Start with the Youth Curator Movement:**
- Explore free, open-source learning materials
- Try the project templates
- Build something that matters to you
- Document your community's stories

**Get Started:**
1. Visit [Youth Curator Movement](youth-curator-movement.md)
2. Try your first AI prompt (it's in there!)
3. Pick a project that interests you
4. Share what you create

**No experience needed. Just curiosity.**

---

## 🎨 For Creators & Brands

### Learn Our Frameworks

**Explore the open-source tools:**
- [Quantum Brand Engine](cookbooks/business/quantum-brand-engine.md)
- [AI Discovery (AAO)](cookbooks/business/ai-discovery-singularity.md)
- [Creator Singularity Engine](cookbooks/business/creator-engine.md)
- [Schema.org Mastery](cookbooks/business/schema-mastery.md)

**Use them. Adapt them. Build with them.**

### Collaborate on Projects

**Want to work together?**

We collaborate with brands and creators who:
- Prioritize greater good over profit maximization
- Value ethical AI practices
- Want to build transparently
- Serve communities authentically

**Reach out:** founders@curations.org *(coming soon)*

---

## 💻 For Developers & Engineers

### Contribute Code & Frameworks

**What we welcome:**
- Improvements to existing frameworks
- New Curator definitions
- Technical optimizations
- Documentation enhancements
- Bug fixes and feature additions

**How to contribute:**
1. Explore the [GitHub repository](https://github.com/curationsla/ai-learning)
2. Open an issue or discussion
3. Fork and create a pull request
4. Follow contribution guidelines

**Contribution Guidelines:**
- Code should align with [our principles](about-curations.md)
- Documentation for all new features
- Tests where appropriate
- Respectful collaboration

### Build With CURATIONS

**Use our frameworks in your projects:**
- All frameworks are open-source
- Attribution appreciated but not required
- Share what you build (optional)
- Contribute improvements back (encouraged)

---

## 📚 For Educators

### Use the Curriculum

**The Youth Curator Movement is for you:**
- Free, complete curriculum
- Lesson plans and assessments
- Project templates
- Professional development resources

**How to start:**
1. Review the [Youth Curator Movement](youth-curator-movement.md)
2. Download curriculum materials *(coming soon)*
3. Adapt for your classroom
4. Share feedback and improvements

**We're here to support you.**

### Contribute Educational Materials

**Help us improve:**
- Share lesson plans that worked
- Suggest age-appropriate adaptations
- Contribute project ideas
- Report what didn't work (so we can fix it)

**Contact:** education@curations.org *(coming soon)*

---

## 🏘️ For Communities & Nonprofits

### Tell Your Story

**Use our frameworks for community storytelling:**
- Local journalism
- Oral history projects
- Cultural documentation
- Community organizing

**We provide:**
- Free frameworks and tools
- Technical guidance
- Pro-bono support (capacity permitting)
- Platform access (CurationsLA model)

**Interested?** community@curations.org *(coming soon)*

### Partner With Us

**We work with:**
- Community organizations
- Neighborhood councils
- Cultural institutions
- Local media initiatives
- Nonprofit organizations

**What we offer:**
- Human × AI collaboration models
- Content and storytelling systems
- Technical implementation
- Training and capacity building

**Let's talk:** partnerships@curations.org *(coming soon)*

---

## 🔬 For Researchers

### Study Our Methods

**Everything we build is documented:**
- Multi-agent architectures
- Prompt engineering techniques
- Ethical AI frameworks
- Human-AI collaboration patterns

**Use our work:**
- Cite our frameworks
- Build on our research
- Test our methods
- Share your findings

**Collaboration opportunities:**
- Research partnerships
- Case studies
- Academic publications
- Conference presentations

**Contact:** research@curations.org *(coming soon)*

---

## Ways to Contribute

### 1. **Share Knowledge**

**What helps:**
- Write about your experience using CURATIONS frameworks
- Create tutorials or guides
- Share case studies
- Document your process

**Where to share:**
- Your own blog or platform
- Social media (tag @curations)
- Community discussions *(coming soon)*
- GitHub discussions

### 2. **Improve Documentation**

**What we need:**
- Clearer explanations
- More examples
- Typo fixes
- Broken link reports
- Missing information flagged

**How to help:**
- Open GitHub issues
- Submit pull requests
- Suggest improvements
- Point out confusion

### 3. **Build Extensions**

**What you can create:**
- New Curator personas
- Framework extensions
- Integration tools
- Example implementations
- Templates and starters

**Share them:**
- GitHub repository
- Community showcase *(coming soon)*
- Your own platform

### 4. **Spread the Word**

**Help others find CURATIONS:**
- Share on social media
- Tell colleagues and friends
- Link from your projects
- Present at events

**Especially share with:**
- Youth who'd benefit from the Movement
- Educators looking for AI curricula
- Developers building with AI
- Communities telling their stories

### 5. **Give Feedback**

**What helps us improve:**
- What worked for you
- What didn't work
- What's missing
- What confused you
- What surprised you

**How to share:**
- GitHub issues
- Email (coming soon)
- Community forums *(coming soon)*

---

## Community Guidelines

### Our Values

When participating in CURATIONS community:

**✅ DO:**
- Be respectful and kind
- Assume good intent
- Help others learn
- Share knowledge openly
- Give credit where due
- Prioritize substance over hype
- Focus on building, not tearing down

**❌ DON'T:**
- Harass or demean others
- Share others' private information
- Spam or self-promote excessively
- Use the platform for political agendas
- Extract value without contributing back
- Ignore ethical guidelines

### Moderation

We maintain a **good vibes only** environment:
- Civil discourse expected
- Harmful content removed
- Repeated violations result in removal
- We prioritize community wellbeing

---

## Connect

### Stay Updated

**Website:**
[hub.curations.org](https://hub.curations.org)

**GitHub:**
[github.com/curationsla/ai-learning](https://github.com/curationsla/ai-learning)

**Community** *(coming soon)*
- Discussion forums
- Discord / Slack
- Newsletter

### Contact

**General inquiries:**
hello@curations.org *(coming soon)*

**Collaborations:**
partnerships@curations.org *(coming soon)*

**Education:**
education@curations.org *(coming soon)*

**Technical/GitHub:**
Open issues on [GitHub](https://github.com/curationsla/ai-learning)

---

## Frequently Asked Questions

### Can I use CURATIONS frameworks commercially?

Yes. All frameworks are open-source. Attribution appreciated but not required.

### Do I need permission to adapt your materials?

No. Use them, remix them, adapt them. Just maintain the ethical principles.

### Can I contribute even if I'm not technical?

Yes! We need writers, educators, designers, storytellers, and community organizers.

### How do I know if my contribution is valuable?

If it helps someone learn, build, or create ethically with AI - it's valuable.

### Is there a formal contribution process?

For code: GitHub pull requests
For everything else: Start a discussion or send an email

### Do contributors get credit?

Yes. All contributors acknowledged in project documentation and release notes.

### Can I get hired by CURATIONS?

We're a small operation. We don't have formal hiring, but we collaborate with talented people aligned with our mission. Reach out if you're interested.

---

## Start Contributing Today

Pick one thing to do right now:

**If you're a learner:**
→ [Try the Youth Curator Movement](youth-curator-movement.md)

**If you're a builder:**
→ [Explore the Cookbook](cookbooks/README.md) and build something

**If you're an educator:**
→ [Review the curriculum](youth-curator-movement.md) and adapt for your students

**If you're a developer:**
→ [Check the GitHub repo](https://github.com/curationsla/ai-learning) and open an issue or PR

**If you're a community organizer:**
→ Email us about partnership opportunities

**If you're just curious:**
→ Read, explore, share what resonates

---

<!--
EASTER EGG LAYER 8: THE DEVELOPER (ARCHITECT)
For those ready to contribute to core frameworks
-->

<!--
💻 You've explored the whole system. Ready to contribute?

Here's your architect-level challenge:

"Using CURATIONS principles (ethical AI, human-first design, open learning),
design a new framework or Curator that we haven't covered yet.

Your proposal should include:
1. Problem it solves
2. How it aligns with greater good creativity
3. Technical architecture
4. Implementation approach
5. How it integrates with existing Curators
6. Ethical considerations

Document it using the CURATIONS style."

Build it in:
- ChatGPT: https://chat.openai.com
- Claude: https://claude.ai

When ready, submit a PR to: https://github.com/curationsla/ai-learning
-->

*"The best way to predict the future is to build it. Together."*

**Let's build.** 🤝

[← Back to Welcome](../readme.md)
